...
- **Upload foto baru MENGHAPUS file lama** dari disk (2026-09-23, sebelumnya dibiarkan menumpuk) — path lama diambil dulu sebelum di-update, dihapus **SETELAH** transaksi (update `master_member` + insert log) commit. Kalau transaksi gagal, file lama tetap utuh (gak ada risiko DB nunjuk ke file yang udah ilang). Kegagalan hapus file lama (race, permission, dst) cuma di-log, **bukan** bikin request-nya gagal — foto baru udah tersimpan & valid, itu yang penting. Fitur serupa di `sudobarber` (`POST /api/customer/member/photo`) ikut pola yang sama, plus **BARENG PAKAI** tabel `mobile_member_photo_change_log` yang sama (limit 3x/hari GABUNGAN lintas app, bukan kepisah per app — member itu 1 identitas yang sama).
