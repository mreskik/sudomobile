# Account - Top Up Balance

Top-up saldo dompet member yang lagi login. **PROTECTED** (wajib `Authorization: Bearer <token>`, beda dari `/order` yang sekarang publik — lihat [`CREATE ORDER.md`](../ORDER/CREATE%20ORDER.md)) — gak ada skenario top-up tanpa identitas, saldo yang nambah harus jelas kepunyaan siapa.
...
Saldo **BELUM** berubah di titik ini — `member_balance_ledger` baru diisi begitu payment settlement confirmed (lewat polling `status`, lihat bagian 2).
...
Dipanggil buat **POLLING** (mis. tiap beberapa detik) sambil QR ditampilin ke customer, abis manggil Create di atas. Mirror alur [`PAYMENT STATUS.md`](../ORDER/PAYMENT%20STATUS.md) order, plus **cek kepemilikan** (`member_topup_online.member_id` harus cocok member yang login) — top-up SELALU protected (beda dari order yang sekarang publik via `order_number`), jadi guard ini tetap ada.
...
- `sudomobile/backend/modules/topup/topup_service.go` — `CreateTopup()`, `CheckTopupStatus()`, `confirmTopupPaid()`, `lockMemberAndInsertLedger()`.
- `sudomobile/backend/modules/topup/topup_handler.go` — `Create()`, `CheckStatus()`.
...
- Referensi/pola: `APIANDORDER/backend/modules/apipos/membertopup/membertopup_service.go` (`lockMemberAndInsertLedger()`, `confirmTopupPaid()` — logic ledger identik, cuma `source` beda: `'mobile'` di sini vs bisa `'pos'`/`'kiosk'` di situ).
